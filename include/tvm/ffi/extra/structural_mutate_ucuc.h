 ...
 */
/// \cond Doxygen_Suppress
#define TVM_FFI_S_MUTATE_ASSIGN_OR_RETURN_UNCHECKED_IMPL_(Result, Type, Name, ResultExpr) \
  auto Result = (ResultExpr); /* NOLINT(bugprone-macro-parentheses) */                    \
  TVM_FFI_S_MUTATE_MAYBE_EARLY_RETURN(Result);                                            \
  Type Name = /* NOLINT(bugprone-macro-parentheses) */                                    \
      ::tvm::ffi::details::AnyUnsafe::MoveFromAnyAfterCheck<Type>(                        \
...
/// \endcond
/*!
 * \brief TVM_FFI_S_MUTATE_ASSIGN_OR_RETURN without the declared-type check.
 *
 * The caller guarantees the result has type ``Type``; a mismatch is undefined behaviour.
 */
#define TVM_FFI_S_MUTATE_ASSIGN_OR_RETURN_UNCHECKED(Type, Name, ResultExpr)               \
  TVM_FFI_S_MUTATE_ASSIGN_OR_RETURN_UNCHECKED_IMPL_(                                      \
