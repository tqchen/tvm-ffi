 ...
 * ``kTVMFFIUnchanged`` type index.  The class deliberately exposes only the
...
TVM_FFI_INLINE Expected<Any> ResolveStructuralMutateResult(MaybeUnchanged<Any>&& result,
  ...
  if (data.type_index() == TypeIndex::kTVMFFIUnchanged) {
    return Any(original);
  }
...
class StructuralMutatorObj : public Object {
 ...
 protected:
  ...
  TVMFFIAny DefaultMutateRawTail(AnyView value, AnyView attr) noexcept {
    ...
    if (remappable) {
      const Any& result_data = details::ExpectedUnsafe::GetData(result);
      AnyView mapped_value =
          result_data.type_index() == TypeIndex::kTVMFFIUnchanged
              ? value
              : AnyView(result_data);
      Expected<void> set_result = VarRemapSetExpected(value, mapped_value);
...
TVM_FFI_INLINE static Expected<Any> MutateReflectedFieldsExpected(StructuralMutatorObj* mutator,
  ...
  const Any& result_data = details::ExpectedUnsafe::GetData(result);
  Object* new_obj = const_cast<Object*>(result_data.as<Object>());
...
struct StructuralMutateAssignTraits {
  TVM_FFI_INLINE static bool Check(const Any& value) noexcept {
    ...
    return value.type_index() != TypeIndex::kTVMFFIUnchanged &&
  ...
  TVM_FFI_INLINE bool TryLink(Callback& callback, AnyView value, Expected<Any>* out) noexcept {
    ...
    if constexpr (order == WalkOrder::kPreOrder) {
      ...
      *out = [&]() -> Expected<Any> {
      ...
      const AnyView mapped_value =
          descended_value.type_index() == TypeIndex::kTVMFFIUnchanged
              ? value
              : AnyView(descended_value);
      *out = [&]() -> Expected<Any> {
        if constexpr (std::is_same_v<TSub, AnyView>) {
          return InvokeTypedCallbackLink(callback, mapped_value, StateIndices{});
        } else if constexpr (std::is_same_v<TSub, Any>) {
