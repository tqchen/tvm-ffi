...
struct Unchanged {
  /*!
   * \brief Convert this tag to its raw marker representation.
...
class UnchangedOr {
 public:
  ...
  template <typename U, std::enable_if_t<std::is_base_of_v<Error, std::decay_t<U>> ||
                                             details::is_unexpected_v<std::decay_t<U>>,
                                         int> = 0>
  UnchangedOr(U&&) = delete;
...
class StructuralMutatorObj : public Object {
 public:
  ...
  TVM_FFI_INLINE Expected<Any> DefaultMutateExpected(AnyView value) noexcept {
    ...
    if (TVM_FFI_PREDICT_TRUE(attr.type_index() == TypeIndex::kTVMFFIOpaquePtr)) {
      TVMFFIAny raw = (*reinterpret_cast<FStructuralMutate>(attr.cast<void*>()))(this, value);
      if (TVM_FFI_PREDICT_FALSE(raw.type_index == TypeIndex::kTVMFFIError)) {
        details::UpdateVisitErrorContext(raw, value);
      }
      return details::ExpectedUnsafe::MoveFromTVMFFIAny<Any>(raw);
    }
    Expected<Any> result = DefaultMutateExpectedTail(value, attr);
    if (TVM_FFI_PREDICT_FALSE(result.is_err())) {
      TVMFFIAny raw = details::ExpectedUnsafe::MoveToTVMFFIAny(std::move(result));
      details::UpdateVisitErrorContext(raw, value);
      return details::ExpectedUnsafe::MoveFromTVMFFIAny<Any>(raw);
    }
    return result;
  ...
  TVM_FFI_INLINE Expected<Any> DefaultMaybeInplaceMutateExpected(AnyView value) noexcept {
    ...
    return DefaultMaybeInplaceMutateExpectedTail(value, attr);
  }
 ...
 protected:
  ...
  Expected<Any> DefaultMutateExpectedTail(AnyView value, AnyView attr) noexcept {
    if (attr.type_index() != TypeIndex::kTVMFFINone) {
      ...
      return BadStructuralMutateHookError();
    }
    ...
    if (remappable) {
      ...
      AnyView value_to_store =
          result_value.type_index() == TypeIndex::kTVMFFIUnchanged ? value : AnyView(result_value);
   ...
   */
  Expected<Any> DefaultMaybeInplaceMutateExpectedTail(AnyView value, AnyView attr) noexcept {
    if (attr.type_index() == TypeIndex::kTVMFFIFunction) {
      Expected<Any> result = attr.cast<Function>().CallExpected<Any>(this, value);
      if (TVM_FFI_PREDICT_FALSE(result.is_err())) {
        TVMFFIAny raw = details::ExpectedUnsafe::MoveToTVMFFIAny(std::move(result));
        details::UpdateVisitErrorContext(raw, value);
        return details::ExpectedUnsafe::MoveFromTVMFFIAny<Any>(raw);
...
class StructuralMapEngine : public Parent {
 ...
 private:
  ...
  static TVMFFIAny DispatchMaybeInplaceMutate(StructuralMutatorObj* mutator,
    ...
    return details::ExpectedUnsafe::MoveToTVMFFIAny(self->MaybeInplaceMutateImplExpected(value));
  }
  ...
  static TVMFFIAny DispatchMutate(StructuralMutatorObj* mutator, AnyView value) noexcept {
    auto* self = static_cast<StructuralMapEngine*>(mutator);
    return details::ExpectedUnsafe::MoveToTVMFFIAny(self->MutateImplExpected(value));
  }
  ...
  TVM_FFI_INLINE bool TryLink(Callback& callback, AnyView value, Expected<Any>* out) noexcept {
    ...
    if (remappable) {
      ...
      if (mapped.is_err() ||
          ExpectedUnsafe::GetData(mapped).type_index() != TypeIndex::kTVMFFINone) {
        *out = std::move(mapped);
        return true;
      }
    }
   ...
   * \return Raw ``TVMFFIAny`` containing the mutated value or Error.
   */
  TVM_FFI_INLINE Expected<Any> MutateImplExpected(AnyView value) noexcept {
    Expected<Any> out{Any()};
    if (TryLinks<false>(value, &out, std::index_sequence_for<Callbacks...>{})) {
      return out;
    }
    return this->DefaultMutateExpected(value);
  }
   ...
   * \return Raw ``TVMFFIAny`` containing the mutated value or Error.
   */
  TVM_FFI_INLINE Expected<Any> MaybeInplaceMutateImplExpected(AnyView value) noexcept {
    Expected<Any> out{Any()};
...
class StructuralMapDynEngine : public Parent {
 ...
 private:
  ...
  static TVMFFIAny DispatchMaybeInplaceMutate(StructuralMutatorObj* mutator,
                                              AnyView value) noexcept {
    return details::ExpectedUnsafe::MoveToTVMFFIAny(static_cast<StructuralMapDynEngine*>(mutator)->MaybeInplaceMutateImplExpected(value));
  }

  /*! \brief Dispatch non-in-place mutation through the ABI vtable. */
  static TVMFFIAny DispatchMutate(StructuralMutatorObj* mutator, AnyView value) noexcept {
    return details::ExpectedUnsafe::MoveToTVMFFIAny(static_cast<StructuralMapDynEngine*>(mutator)->MutateImplExpected(value));
  }
...
class StructuralMutateEngine : public Parent {
 ...
 private:
  ...
  static TVMFFIAny DispatchMutate(StructuralMutatorObj* mutator, AnyView value) noexcept {
    return details::ExpectedUnsafe::MoveToTVMFFIAny(static_cast<StructuralMutateEngine*>(mutator)->MutateImplExpected(value));
  }

  /*! \brief Dispatch maybe-in-place mutation from the erased mutator pointer. */
  static TVMFFIAny DispatchMaybeInplaceMutate(StructuralMutatorObj* mutator,
                                              AnyView value) noexcept {
    return details::ExpectedUnsafe::MoveToTVMFFIAny(static_cast<StructuralMutateEngine*>(mutator)->MaybeInplaceMutateImplExpected(value));
  }

  /*! \brief Mutate one value, handing a matched callback ownership of descent. */
  Expected<Any> MutateImplExpected(AnyView value) noexcept {
    ...
    return Parent::DefaultMutateExpected(value);
  }

  /*! \brief Maybe mutate one value in place, with callback-owned descent. */
  Expected<Any> MaybeInplaceMutateImplExpected(AnyView value) noexcept {
    ...
    return Parent::DefaultMaybeInplaceMutateExpected(value);
  }
