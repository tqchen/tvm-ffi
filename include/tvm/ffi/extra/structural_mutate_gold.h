...
template <typename T>
class MaybeUnchanged;

namespace details {
struct MaybeUnchangedUnsafe;

TVM_FFI_INLINE Any StructuralMutateUnchangedValue() noexcept {
  TVMFFIAny raw{};
  raw.type_index = TypeIndex::kTVMFFIStructuralMutateUnchanged;
  return AnyUnsafe::MoveTVMFFIAnyToAny(&raw);
}
}  // namespace details

/*!
 * \brief A structural-mutation result containing a replacement, an Error, or no new value.
 *
 * The unchanged state is encoded in the existing ``Any`` payload with the internal
 * ``kTVMFFIStructuralMutateUnchanged`` type index.  The class deliberately exposes only the
 * operations needed by structural-mutation hooks, and cannot be substituted for an
 * ``Expected<T>``.
 */
template <typename T>
class MaybeUnchanged {
 public:
  static_assert(!std::is_void_v<T>, "MaybeUnchanged<void> is not supported");
  static_assert(!std::is_same_v<T, Error>, "MaybeUnchanged<Error> is not supported");

  /*! \brief Construct an unchanged result. */
  TVM_FFI_INLINE MaybeUnchanged() noexcept : data_(details::StructuralMutateUnchangedValue()) {}

  // NOLINTNEXTLINE(google-explicit-constructor,runtime/explicit)
  TVM_FFI_INLINE MaybeUnchanged(T value) : data_(Any(std::move(value))) {}

  // NOLINTNEXTLINE(google-explicit-constructor,runtime/explicit)
  TVM_FFI_INLINE MaybeUnchanged(Error error) : data_(Any(std::move(error))) {}

  template <typename E, typename = std::enable_if_t<std::is_base_of_v<Error, std::remove_cv_t<E>>>>
  // NOLINTNEXTLINE(google-explicit-constructor,runtime/explicit)
  TVM_FFI_INLINE MaybeUnchanged(Unexpected<E> unexpected)
      : data_(Any(std::move(unexpected).error())) {}

  /*! \brief Whether the result contains an Error. */
  TVM_FFI_INLINE bool is_err() const& noexcept {
    return data_.type_index() == TypeIndex::kTVMFFIError;
  }
  /*! \brief Whether the result asks the caller to preserve the original value. */
  TVM_FFI_INLINE bool unchanged() const& noexcept {
    return data_.type_index() == TypeIndex::kTVMFFIStructuralMutateUnchanged;
  }

  /*! \brief Whether the result is unchanged or contains the original object identity. */
  TVM_FFI_INLINE bool unchanged_or_same_as(const T& original) const& noexcept {
    return unchanged() || data_.same_as(original);
  }

  /*!
   * \brief Move out the replacement, or move \p original when unchanged.
   * \pre The caller has already handled the error state.
   */
  TVM_FFI_INLINE T value_or_unchanged(T& original) && {
    if (unchanged()) {
  ...
  /*!
   * \brief Move the replacement into \p dst when changed.
   * \return Whether \p dst was written.
   * \pre The caller has already handled the error state.
   */
  TVM_FFI_INLINE bool move_if_changed(T* dst) && {
    if (unchanged()) {
      return false;
    }
    *dst = details::AnyUnsafe::MoveFromAnyAfterCheck<T>(std::move(data_));
    return true;
  }

 private:
  friend struct details::MaybeUnchangedUnsafe;
  struct UnsafeInit {};
  TVM_FFI_INLINE MaybeUnchanged(UnsafeInit, Any&& data) noexcept : data_(std::move(data)) {}
  Any data_;
};

static_assert(sizeof(MaybeUnchanged<Any>) == sizeof(Expected<Any>));
static_assert(alignof(MaybeUnchanged<Any>) == alignof(Expected<Any>));
static_assert(std::is_trivially_destructible_v<MaybeUnchanged<Any>> ==
              std::is_trivially_destructible_v<Expected<Any>>);
static_assert(std::is_trivially_copy_constructible_v<MaybeUnchanged<Any>> ==
              std::is_trivially_copy_constructible_v<Expected<Any>>);
static_assert(std::is_trivially_move_constructible_v<MaybeUnchanged<Any>> ==
...
/*! \brief Unsafe moves between MaybeUnchanged and its single Any storage. */
struct MaybeUnchangedUnsafe {
  template <typename T>
  TVM_FFI_INLINE static MaybeUnchanged<T> MoveFromAny(Any&& value) noexcept {
    return MaybeUnchanged<T>(typename MaybeUnchanged<T>::UnsafeInit{}, std::move(value));
  }

  /*! \brief Move a raw ABI result straight into the protocol type, with no intermediate. */
  template <typename T>
  TVM_FFI_INLINE static MaybeUnchanged<T> MoveFromTVMFFIAny(TVMFFIAny raw) noexcept {
    return MaybeUnchanged<T>(typename MaybeUnchanged<T>::UnsafeInit{},
                             AnyUnsafe::MoveTVMFFIAnyToAny(&raw));
  }

  template <typename T>
  TVM_FFI_INLINE static Any&& MoveToAny(MaybeUnchanged<T>& result) noexcept {
    return std::move(result.data_);
  }

  /*! \brief Expose the single Any storage without moving it. */
  template <typename T>
  TVM_FFI_INLINE static const Any& GetData(const MaybeUnchanged<T>& result) noexcept {
    return result.data_;
  }
};

template <typename Result>
struct SMutateResultTraits;

// Keep extraction and early-return classification in one per-result dispatch point.
template <typename T>
struct SMutateResultTraits<Expected<T>> {
  TVM_FFI_INLINE static Any&& MoveToAny(Expected<T>& result) noexcept {
    return ExpectedUnsafe::GetData(result);
  }

  TVM_FFI_INLINE static bool ShouldEarlyReturn(int32_t type_index) noexcept {
    return type_index == TypeIndex::kTVMFFIError;
  }
};

template <typename T>
struct SMutateResultTraits<MaybeUnchanged<T>> {
  TVM_FFI_INLINE static Any&& MoveToAny(MaybeUnchanged<T>& result) noexcept {
    return MaybeUnchangedUnsafe::MoveToAny(result);
  }
...
/*! \brief Resolve the unchanged protocol into the legacy owning-result contract.
 *
 * This compatibility conversion necessarily constructs an owning ``Any`` from \p original when
 * no value changed: ``Expected<Any>`` has no unchanged state. It is transitional while hook
 * authors migrate to the direct ``MaybeUnchanged`` protocol, where the refcount-saving benefit
 * remains intact.
 */
template <typename T>
TVM_FFI_INLINE Expected<Any> ResolveStructuralMutateResult(MaybeUnchanged<Any>&& result,
                                                           const T& original) noexcept {
  Any&& data = MaybeUnchangedUnsafe::MoveToAny(result);
  if (data.type_index() == TypeIndex::kTVMFFIStructuralMutateUnchanged) {
    return Any(original);
  }
...
class StructuralMutatorObj : public Object {
 public:
   ...
   * \return The replacement, an Error, or the unchanged marker.
   *
   * This is the descent. There is exactly one, and it speaks the unchanged protocol: a hook or
   * a reflected walk that produced nothing new says so instead of handing back a fresh owning
   * copy of what the caller already holds. Returning a value is always valid, so a hook that
   * never reports unchanged is a correct hook.
   *
   * This entry point never intentionally mutates \p value in place. Recursive mutations
   * also use \ref Mutate.
   */
  TVM_FFI_INLINE MaybeUnchanged<Any> Mutate(AnyView value) noexcept {
    return details::MaybeUnchangedUnsafe::MoveFromTVMFFIAny<Any>((*vtable_->mutate)(this, value));
  }

  /*! \brief Typed form of \ref Mutate, avoiding an AnyView round trip at the call site. */
  template <typename T, typename = std::enable_if_t<std::is_base_of_v<ObjectRef, T>>>
  TVM_FFI_INLINE MaybeUnchanged<Any> Mutate(const T& value) noexcept {
    return details::MaybeUnchangedUnsafe::MoveFromTVMFFIAny<Any>(
        (*vtable_->mutate)(this, AnyView(value)));
  }

  /*!
   * \brief Compatibility form of \ref Mutate for a caller that wants an owning result.
   *
   * \param value The value to mutate.
   * \return The mutated owning value, or an Error if mutation failed.
   *
   * A thin wrapper over the one descent, not a second one: it converts the single result it is
   * given. An unmigrated hook returns a value and never the marker, so for such a caller this
   * is a type-index test that is always false plus a move -- no reconstitution and no refcount.
   * The owning copy is built only when a migrated hook's unchanged result reaches an unmigrated
   * caller, which is the exact cost of straddling the two and is paid by whoever straddles.
   */
  TVM_FFI_INLINE Expected<Any> MutateExpected(AnyView value) noexcept {
    return details::ResolveStructuralMutateResult(Mutate(value), value);
  }

  /*! \brief Typed form of \ref MutateExpected. */
  template <typename T, typename = std::enable_if_t<std::is_base_of_v<ObjectRef, T>>>
  TVM_FFI_INLINE Expected<Any> MutateExpected(const T& value) noexcept {
    return details::ResolveStructuralMutateResult(Mutate(value), value);
  }
   ...
   * \return The replacement, an Error, or the unchanged marker.
   *
   * A returned value may refer to the same object as \p value. Callers must use the result as
   * the result of the mutation rather than assuming that the input object was reused.
   *
   ...
   */
  TVM_FFI_INLINE MaybeUnchanged<Any> MaybeInplaceMutate(AnyView value) noexcept {
    return details::MaybeUnchangedUnsafe::MoveFromTVMFFIAny<Any>(
        (*vtable_->maybe_inplace_mutate)(this, value));
  }

  /*! \brief Typed form of \ref MaybeInplaceMutate. */
  template <typename T, typename = std::enable_if_t<std::is_base_of_v<ObjectRef, T>>>
  TVM_FFI_INLINE MaybeUnchanged<Any> MaybeInplaceMutate(const T& value) noexcept {
    return details::MaybeUnchangedUnsafe::MoveFromTVMFFIAny<Any>(
        (*vtable_->maybe_inplace_mutate)(this, AnyView(value)));
  }

  /*! \brief Compatibility form of \ref MaybeInplaceMutate; see \ref MutateExpected. */
  TVM_FFI_INLINE Expected<Any> MaybeInplaceMutateExpected(AnyView value) noexcept {
    return details::ResolveStructuralMutateResult(MaybeInplaceMutate(value), value);
  }

  /*! \brief Typed form of \ref MaybeInplaceMutateExpected. */
  template <typename T, typename = std::enable_if_t<std::is_base_of_v<ObjectRef, T>>>
  TVM_FFI_INLINE Expected<Any> MaybeInplaceMutateExpected(const T& value) noexcept {
    return details::ResolveStructuralMutateResult(MaybeInplaceMutate(value), value);
  }
   ...
   */
  TVM_FFI_INLINE MaybeUnchanged<Any> MaybeInplaceMutateIfUnique(AnyView value) noexcept {
    const Object* obj = value.as<Object>();
    if (obj != nullptr && obj->unique()) return MaybeInplaceMutate(value);
    return Mutate(value);
  }

  /*! \brief Typed form of \ref MaybeInplaceMutateIfUnique. */
  template <typename T, typename = std::enable_if_t<std::is_base_of_v<ObjectRef, T>>>
  TVM_FFI_INLINE MaybeUnchanged<Any> MaybeInplaceMutateIfUnique(const T& value) noexcept {
    const Object* obj = value.get();
    if (obj != nullptr && obj->unique()) return MaybeInplaceMutate(value);
    return Mutate(value);
  }

  /*! \brief Compatibility form of \ref MaybeInplaceMutateIfUnique; see \ref MutateExpected. */
  ...
  /*! \brief Typed form of \ref MaybeInplaceMutateIfUniqueExpected. */
  template <typename T, typename = std::enable_if_t<std::is_base_of_v<ObjectRef, T>>>
  ...
  /*!
   * \brief Raw form of \ref VarRemapGetExpected, for the descent's own lookup.
   * \param var The borrowed variable identity to look up.
   * \return Raw ``TVMFFIAny`` holding the stored replacement, FFI None on a miss, or an Error.
   *
   * The environment stores values, not protocol state. On a hit the stored value is already the
   * answer, so the descent forwards it: the caller reads one type index to tell a miss (descend)
   * from everything else (this is the result), and moves. A miss leaves a None ``TVMFFIAny``,
   * which owns nothing, so the miss path constructs and destroys no ``Expected<Any>`` at all.
   */
  TVM_FFI_INLINE TVMFFIAny VarRemapGetRaw(AnyView var) noexcept {
    return (*vtable_->var_remap_get)(this, var);
  }
 ...
 protected:
  ...
  TVMFFIAny DefaultMutateRawTail(AnyView value, AnyView attr) noexcept {
    ...
    if (remappable) {
      // Only None means "no entry, descend". A hit is the answer and so is an Error, and both
      // are already in the right shape, so they are forwarded rather than unwrapped and rebuilt.
      TVMFFIAny mapped = VarRemapGetRaw(value);
      if (mapped.type_index != TypeIndex::kTVMFFINone) return mapped;
    }
    ...
    if (remappable) {
      ...
      AnyView value_to_store =
          result_value.type_index() == TypeIndex::kTVMFFIStructuralMutateUnchanged
              ? value
...
TVM_FFI_INLINE static Expected<Any> MutateReflectedFieldsExpected(StructuralMutatorObj* mutator,
  ...
  auto mutate_fields = [&]() {
    reflection::ForEachFieldInfoWithEarlyStop(
        type_info, [&](const TVMFFIFieldInfo* field_info) -> bool {
          ...
          // The reflected walk is a descent like any other, so it uses the descent protocol:
          // an unchanged field says so, and this loop skips it without the owning copy of
          // `field_value` that a resolving entry point would have had to build to say the
          // same thing. That copy was paid on every unchanged field of every walked node.
          MaybeUnchanged<Any> mutated_field = [&]() -> MaybeUnchanged<Any> {
            if (field_info->flags & kTVMFFIFieldFlagBitMaskSEqHashDefNonRecursive) {
              return mutator->WithDefRegionKind(kTVMFFIDefRegionKindNonRecursive,
                                                [&]() { return mutator->Mutate(field_value); });
          ...
          const Any& mutated_field_data = details::MaybeUnchangedUnsafe::GetData(mutated_field);
          if (TVM_FFI_PREDICT_FALSE(mutated_field_data.type_index() == TypeIndex::kTVMFFIError)) {
            result = Unexpected(mutated_field_data.cast<Error>());
            return true;
          }
          ...
          if (mutated_field_data.type_index() == TypeIndex::kTVMFFIStructuralMutateUnchanged ||
              field_value.same_as(mutated_field_data)) {
            return false;
          }
          const Any& new_field = mutated_field_data;
  ...
  if (!field_changed) {
    return details::StructuralMutateUnchangedValue();
  }
  return result;
}
...
/// \cond Doxygen_Suppress
#define TVM_FFI_S_MUTATE_DATA_OR_RETURN_IMPL_(Result, Data, ShouldReturn)            \
  using TVM_FFI_STR_CONCAT(Data, traits_) =                                          \
      ::tvm::ffi::details::SMutateResultTraits<::std::decay_t<decltype(Result)>>;    \
  auto&& Data = TVM_FFI_STR_CONCAT(Data, traits_)::MoveToAny(Result);                \
  const int32_t TVM_FFI_STR_CONCAT(Data, type_index_) = Data.type_index();           \
  if (ShouldReturn) {                                                                \
    return ::tvm::ffi::details::MaybeReturnHelper(                                   \
        ::tvm::ffi::details::ExpectedUnsafe::MoveFromTVMFFIAny<::tvm::ffi::Any>(     \
            ::tvm::ffi::details::AnyUnsafe::MoveAnyToTVMFFIAny(::std::move(Data)))); \
  }

// Return from the current raw or Expected<Any> mutation function on Error. An unchanged child
// does not imply that the enclosing node is unchanged; its remaining fields must still be visited.
#define TVM_FFI_S_MUTATE_MAYBE_EARLY_RETURN(Result)                                 \
  do {                                                                              \
    auto&& tvm_ffi_res_ = (Result);                                                 \
    TVM_FFI_S_MUTATE_DATA_OR_RETURN_IMPL_(                                          \
        tvm_ffi_res_, tvm_ffi_res_data_,                                            \
        tvm_ffi_res_data_traits_::ShouldEarlyReturn(tvm_ffi_res_data_type_index_)); \
  } while (0)
...
struct StructuralMutateAssignTraits {
  TVM_FFI_INLINE static bool Check(const Any& value) noexcept {
    // Any normally accepts every type.  The protocol marker is deliberately not an ordinary
    // value, so an unconverted destination must reject it explicitly.
    return value.type_index() != TypeIndex::kTVMFFIStructuralMutateUnchanged &&
           AnyUnsafe::CheckAnyStrict<T>(value);
  }

  TVM_FFI_INLINE static T Move(Any&& value) noexcept {
    return AnyUnsafe::MoveFromAnyAfterCheck<T>(std::move(value));
  }
};

template <typename T>
struct StructuralMutateAssignTraits<MaybeUnchanged<T>> {
  TVM_FFI_INLINE static bool Check(const Any& value) noexcept {
    return value.type_index() == TypeIndex::kTVMFFIStructuralMutateUnchanged ||
           AnyUnsafe::CheckAnyStrict<T>(value);
  }
...
// Takes nothing on purpose. Naming the offending type in the message would mean handing this
// cold function the value that the always-false guard in front of it holds, which keeps that
// value live across the call in the hot path that never takes the branch -- the same defect as
// an UpdateVisitErrorContext that takes an AnyView. The declared type is in the source line the
// diagnostic points at, so the value buys little and costs the guard.
...
#define TVM_FFI_S_MUTATE_ASSIGN_OR_RETURN_IMPL_(Result, Type, Name, ResultExpr)              \
  ...
  TVM_FFI_S_MUTATE_DATA_OR_RETURN_IMPL_(                                                     \
      Result, TVM_FFI_STR_CONCAT(Result, _data_),                                            \
      TVM_FFI_STR_CONCAT(Result, _data_type_index_) == ::tvm::ffi::TypeIndex::kTVMFFIError); \
  if (TVM_FFI_PREDICT_FALSE(!::tvm::ffi::details::StructuralMutateAssignTraits<Type>::Check( \
          TVM_FFI_STR_CONCAT(Result, _data_)))) {                                            \
    return ::tvm::ffi::details::MaybeReturnHelper(                                           \
        ::tvm::ffi::details::SMutateDeclaredTypeError());                                    \
  }                                                                                          \
  Type Name = /* NOLINT(bugprone-macro-parentheses) */                                       \
      ::tvm::ffi::details::StructuralMutateAssignTraits<Type>::Move(                         \
          ::std::move(TVM_FFI_STR_CONCAT(Result, _data_)))
/// \endcond
...
#define TVM_FFI_UNSAFE_S_MUTATE_ASSIGN_OR_RETURN_SKIP_CHECK_IMPL_(Result, Type, Name, ResultExpr) \
  ...
  TVM_FFI_S_MUTATE_DATA_OR_RETURN_IMPL_(                                                          \
      Result, TVM_FFI_STR_CONCAT(Result, _data_),                                                 \
      TVM_FFI_STR_CONCAT(Result, _data_type_index_) == ::tvm::ffi::TypeIndex::kTVMFFIError);      \
  TVM_FFI_DCHECK(::tvm::ffi::details::StructuralMutateAssignTraits<Type>::Check(                  \
      TVM_FFI_STR_CONCAT(Result, _data_)))                                                        \
...
class StructuralMapEngine : public Parent {
 ...
 private:
  ...
  TVM_FFI_INLINE bool TryLink(Callback& callback, AnyView value, Expected<Any>* out) noexcept {
    ...
    // A statically non-remappable type whose subclasses cannot change kind discards the remap
    // path at optimization time. Every other case uses runtime metadata: nullable refs may match
    // None, non-final subclasses may redeclare the kind, and metadata may be absent.
    ...
    if (remappable) {
      // Only None means "no entry, descend". A hit is the answer and so is an Error, and both
      // are already in the right shape, so they move straight through: one type-index test and
      // a move, with no unwrap, no re-wrap, no compare against the input and no marker.
      TVMFFIAny mapped = this->VarRemapGetRaw(value);
      if (mapped.type_index != TypeIndex::kTVMFFINone) {
        *out = ExpectedUnsafe::MoveFromTVMFFIAny<Any>(mapped);
        return true;
      }
    }
    ...
    if constexpr (order == WalkOrder::kPreOrder) {
      ...
      *out = [&]() -> Expected<Any> {
      ...
      if (ExpectedUnsafe::GetData(*out).type_index() ==
          TypeIndex::kTVMFFIStructuralMutateUnchanged) {
      ...
      // Two branches, and the unchanged one constructs nothing. The original is already in
      // hand as `value`, so a borrowed view of it is everything the callback needs; an owning
      // `Any(value)` here is a construction and a refcount on every unchanged node that reaches
      // a callback -- the per-node cost this protocol exists to remove, paid back at exactly the
      // point it was saved.
      const Any& descended_value = ExpectedUnsafe::GetData(descended);
      const AnyView mapped_view =
          descended_value.type_index() == TypeIndex::kTVMFFIStructuralMutateUnchanged
              ? value
    ...
    // Unchanged is the dominant case in the workload this protocol exists for, so it is the
    // first branch and it does nothing beyond the binding. Testing for the marker already
    // excludes an error, which has its own type index -- an `is_ok()` in front of it is a second
    // comparison that cannot change the answer, and every path into this tail has already
    // returned on error.
    if (TVM_FFI_PREDICT_TRUE(ExpectedUnsafe::GetData(*out).type_index() ==
                             TypeIndex::kTVMFFIStructuralMutateUnchanged)) {
    ...
    if (remappable) {
      Expected<void> set_result =
          this->VarRemapSetExpected(value, AnyView(ExpectedUnsafe::GetData(*out)));
      if (TVM_FFI_PREDICT_FALSE(set_result.is_err())) {
        *out = Unexpected(std::move(set_result).error());
      }
    }
    ...
    if constexpr (order == WalkOrder::kPreOrder) {
      ...
      *out = [&]() -> Expected<Any> {
      ...
      if (TVM_FFI_PREDICT_FALSE(
              !details::RuntimeTypeIndexMatch(mapped_view.type_index(), link_type_index))) {
 ...
 *       ``__s_maybe_inplace_mutate__`` hook, a nested owned field follows the idiom
