 ...
 */
// BRANCH-LOCAL (bench/tvm-hook): the OLD engine -- upstream main e74e58f's
// include/tvm/ffi/extra/structural_mutate.h verbatim, the pre-UnchangedOr engine with #760 and
// #761 in its base, renamed so it can sit beside the UC, GOLD and mixed engines in one tree.
// The include guard is deliberately the original TVM_FFI_EXTRA_STRUCTURAL_MUTATE_H_: an
// executable that includes this header first makes tvm_hook_override.h's own
// `#include <tvm/ffi/extra/structural_mutate.h>` a no-op, so the OLD hook file compiles
// against this engine without being edited. Nothing below this banner is changed.
