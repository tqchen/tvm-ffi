...
class PrimType final : public Type {
 public:
  explicit PrimType(DLDataType dtype) : Type(UnsafeInit{}) {
...
class TypedExpr : public Expr {
 public:
  ExpectedType ty() const {
    const auto* node = get();
    return GetRef<ExpectedType>(node->ExprNode::ty.template as<typename ExpectedType::ContainerType>());
  }
...
struct TypeTraits<testing::tvmast::PrimExpr>
    : public TvmAstTypedExprWithFallbackTraitsBase<testing::tvmast::PrimExpr,
                                                   testing::tvmast::PrimType, StrictBool, int64_t,
                                                   double, String,
                                                   testing::tvmast::PrimExprConvertible> {
  using Base = TvmAstTypedExprWithFallbackTraitsBase<testing::tvmast::PrimExpr,
                                                     testing::tvmast::PrimType, StrictBool, int64_t,
                                                     double, String,
                                                     testing::tvmast::PrimExprConvertible>;
  ...
  static testing::tvmast::PrimExpr ConvertFallbackValue(
...
class PrimVar : public PrimExpr {
 public:
  explicit PrimVar(String name, PrimType dtype = PrimType::Int(32), Span span = Span())
      : PrimExpr(Var(std::move(name), std::move(dtype), std::move(span))
...
template <typename T>
struct HasRawMutate<T, std::void_t<decltype(std::declval<T*>()->MutateRaw(
                           std::declval<AnyView>()))>> : std::true_type {};
...
using DependentMutatorObj = std::conditional_t<(sizeof(TNode) > 0), StructuralMutatorObj,
...
#define TVM_FFI_TVMAST_REGISTER_BINOP(Name)                                                     \
  refl::TypeAttrDef<Name##Node>()                                                               \
      .attr(refl::type_attr::kStructuralVisit,                                                  \
            reinterpret_cast<void*>(&VisitBinary<Name##Node>))                                  \
