...
// BRANCH-LOCAL (bench/tvm-hook): upstream main e74e58f's src/ffi/extra/structural_mutate.cc
// verbatim apart from this include, compiled by build_mix.sh into the `old` executables only
// (it is not in the library's source list): the OLD engine descends into containers through the
// registered Array/Map/List/Dict hooks, and the library's are UC's, which answer with the
// unchanged marker this engine does not know. Its static-init block re-registers the OLD
// container hooks in the executable, over the library's, which the benchmark build permits.
...
TVM_FFI_STATIC_INIT_BLOCK() {
  ...
  // BRANCH-LOCAL: the ObjectDef<StructuralMutatorObj>() and GlobalDef() registrations of the
  // original block are omitted here -- the library has already registered the type key and the
  // ffi.StructuralMutator* globals, and re-registering a type key aborts at start-up. Only the
  // container hook registrations below are re-done, over the library's, which is what the
  // OLD engine needs.
