 ...
 * under the License.
 */
#ifndef TVM_FFI_BENCHMARKS_CPP_STRUCTURAL_TVM_OVERRIDE_UC_H_
#define TVM_FFI_BENCHMARKS_CPP_STRUCTURAL_TVM_OVERRIDE_UC_H_

// UC's hooks for the split-fuse fixture, against UC's engine (structural_mutate.h).
//
// EXTRACTED FROM bench/377-four-state
// 220f363:benchmarks/cpp/structural/tvm_hook_override_unchanged_or_guarded.h (blob 993c27b22fdf,
// sha256 6b39cad8bee2a4337e0dfbe6ca08b225fe185348c4da02dbf883cfebece49a5c) -- the UC*g*
// construction, which keeps the store off the unchanged path at every in-place assignment site
// with `if (!x.IsUnchanged()) slot = std::move(x).ValueUnchecked();`. That is the inline fix
// #386 requires the UC hook to carry. This file is that file's text with the sections the
// split-fuse fixture never reaches left out -- Call, SeqStmt, Evaluate -- and nothing else
// changed:
//   * the include guard is this file's;
...
// Every remaining line is the original text.
//
...
TVMFFIAny BinaryMutate(ffi::StructuralMutatorObj* mutator, ffi::AnyView value) noexcept {
  ...
  const TNode* self =
      ffi::details::AnyUnsafe::RawObjectPtrFromAnyViewAfterCheck<const TNode>(value);
  TVM_FFI_S_MUTATE_ASSIGN_OR_RETURN(ffi::UnchangedOr<PrimExpr>, a, mutator->MutateExpected(self->a));
  TVM_FFI_S_MUTATE_ASSIGN_OR_RETURN(ffi::UnchangedOr<PrimExpr>, b, mutator->MutateExpected(self->b));
...
#endif  // TVM_FFI_BENCHMARKS_CPP_STRUCTURAL_TVM_OVERRIDE_UC_H_
