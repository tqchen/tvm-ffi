 ...
 * under the License.
 */
#ifndef TVM_FFI_BENCHMARKS_CPP_STRUCTURAL_TVM_HOOK_OVERRIDE_MIXED_H_
#define TVM_FFI_BENCHMARKS_CPP_STRUCTURAL_TVM_HOOK_OVERRIDE_MIXED_H_
...
//
// ROUND-1 CONTROL, unchanged since e865d5c apart from the file rename: the binary-only,
// guarded-store UC hook (`if (!x.IsUnchanged()) slot = ...ValueUnchecked()`) under the mixed
// engine's entry names. tvm_hook_override_uc.h has since moved to the full fixture set and
// the documented `ValueOrUnchanged` idiom; this file deliberately has not, so that the mixed
// executable stays the round-1 control. The sentence below describes the round-1 uc file.
//
// This file is tvm_hook_override_uc.h with the three descent entries renamed to the names the mixed
...
// tvm_hook_override_uc.h's own banner follows, and describes what it is extracted from.
//
...
#endif  // TVM_FFI_BENCHMARKS_CPP_STRUCTURAL_TVM_HOOK_OVERRIDE_MIXED_H_
